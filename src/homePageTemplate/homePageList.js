...
const HomePageList = (props) => (
		...
		<ul className="home_page_search_result_list">
			{
				props.artistList.results.map( item =>
					<li className="home_page_search_resultItem" key={item.id}>
                        <div className="img_wrapper">
                            <Link to={"/artist/" + item.id}>
                                {item.thumb ? <img src={item.thumb} alt="photo" /> : <img src="https://dummyimage.com/150x150/000/fff" alt="photo" />}
                                <h3 className="home_page_search_result_heading">{item.title}</h3>
                            </Link>
                        </div>
					</li>
				)
	...
	) : <p className="home_page_search_result_paragraf">Enter artist</p> );
